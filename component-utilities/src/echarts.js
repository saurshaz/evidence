...
export default (node, option) => {
	...
	const applySeriesColors = () => {
		if (option.seriesColors) {
			...
			if (option.contextmenuEnabled){
				document.addEventListener('contextmenu', (event) => {
					// const _echart = echarts.getInstanceByDom('div#chart-container')  
					// console.log('_echart >> ', _echart)
					return false
				});
			}
	...
	console.log('>>> ***** echarts.js ***** >>> echart initialized ',option)
	...
	// debugger;
	const menu = chart._dom.parentElement.previousElementSibling;
	// const menuOption = chart._dom.closest(".menu-option");
	...
	if (option.contextmenuEnabled){
		chart.on("contextmenu",function (params) {
			...
			console.log(' ***** chart contextmenu params ***** >>> ', params)
			window.__selectedContextMenuParams = params;
			const origin = {
				left: event.pageX,
				top: event.pageY
			};
			setPosition(origin);
			dispatch('contextmenu', params);
			return false;
		});
	}

	// if (option.dblclickEnabled){
		chart.on("dblclick", function (params) {
			console.log('dblclick detected >>', params)
			window.__dblclickParams = params;
			dispatch('dblclick', params);
			option.dblclickHandler(params);
			window.__dblclickParams = '';
		})
	// }
	...
	const updateChart = (newOption) => {
		option = newOption;
		console.log(' **** ***** updateChart from echarts.js *** ')
		chart.setOption(
			{
				...option.config,
				animationDuration: ANIMATION_DURATION,
