...
import type { QueryValue } from './query/Query.js';
export type EventHandler<Events extends Record<string, any> = {}, Event extends keyof Events = keyof Events> = (value: Events[Event], event: Event) => void;
export type EventEmitter<Events extends Record<string, any> = {}> = {
    ...
    removeEventListener<Event extends keyof Events>(event: Event, handler: EventHandler<Events>): void;
...
export type LengthResultRow = {
    rowCount: number;
};
export type MaybeAliasedCol = string | {
    col: string;
    as: string;
};
